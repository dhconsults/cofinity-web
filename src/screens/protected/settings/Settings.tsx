...
export default function SettingsPage() {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>, type: 'logo' | 'cac') => {
  ...
  const saveSettings = async (data: SettingsFormData) => {
    ...
    const logoInput = document.querySelector('input[type="file"][accept="image/*"]') as HTMLInputElement;
    const cacInput = document.querySelector('input[type="file"][accept=".pdf,.jpg,.jpeg,.png"]') as HTMLInputElement;
  ...
  const handleEnableMFA = async () => {
    ...
    try {
      ...
      toast.error('Setup Failed', { description: err?.response?.data?.message || 'Please try again.' });
    } finally {
  ...
  const handleVerifyCode = async (e: React.FormEvent) => {
    ...
    setVerifying(true);
    try {
      await apiClient.post('/api/tenant-users/mfa/enable', { code: verificationCode });
      toast.success('MFA Enabled', { description: 'You will now need a code for withdrawals.' });
      setShowQRModal(false);
      setVerificationCode('');
      // Update user state
      updateUser({ ...user!, mfa_enabled: true });
    } catch (err: any) {
      toast.error('Invalid Code', { description: err?.response?.data?.message || 'Try again.' });
    } finally {
      setVerifying(false);
    }
  };
  ...
  const handleDisableMFA = async () => {
    ...
    try {
      await apiClient.post('/api/tenant-users/mfa/disable', { current_password: password });
      toast.success('MFA Disabled');
      updateUser({ ...user!, mfa_enabled: false });
    } catch (err: any) {
      toast.error('Failed', { description: err?.response?.data?.message || 'Incorrect password' });
    }
  };
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-gray-50">
      {/* Sidebar */}
      <aside className="md:w-64 bg-white border-b md:border-r shadow-sm">
          ...
          {tabs.map((tab) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-left transition-colors ${
                activeTab === tab.key ? 'bg-gray-900 text-white' : 'text-gray-700 hover:bg-gray-100'
              }`}
            >
          ...
          {activeTab === 'organization' && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>Update your cooperative's public information</CardDescription>
              </CardHeader>
              <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <Label>Cooperative Name</Label>
                  <Input {...register('name')} placeholder="e.g. Unity Thrift Cooperative" />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  />
                  {logoPreview && (
                    <img src={logoPreview} alt="Logo preview" className="mt-3 h-24 rounded border" />
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  />
                  {cacPreview && <p className="mt-2 text-sm text-green-600">Certificate uploaded</p>}
                </div>
              </CardContent>
            </Card>
          )}
              ...
              <CardContent className="space-y-6">
                <div>
                  ...
                  <RadioGroup
                    ...
                    onValueChange={(v) => setValue('settlement_type', v as 'collection' | 'wallet')}
                  >
                    <div className="flex items-center space-x-2 mt-3">
                      <RadioGroupItem value="collection" id="collection" />
                      <Label htmlFor="collection" className="font-normal cursor-pointer">
                        Collection Settlement (0.65% fee) – Auto-sweep to wallet daily at 9PM
                      </Label>
                    </div>
                    <div className="flex items-center space-x-2 mt-3">
                      <RadioGroupItem value="wallet" id="wallet" />
                      <Label htmlFor="wallet" className="font-normal cursor-pointer">
                        Direct Wallet Settlement (1% fee) – Instant, max daily withdrawal ₦250,000
                      </Label>
                    </div>
                  </RadioGroup>
                </div>
  ...
  <Card className="p-4">
    <div className="flex justify-between items-center">
      <div>
        <p className="font-medium">{watch('sweep_account_name') || 'Not set'}</p>
        <p className="text-sm text-muted-foreground">
          {watch('sweep_account_number') ? 
            `${watch('sweep_account_number')} • ${watch('sweep_bank_name')}` : 
          ...
          {activeTab === 'fees' && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>Set fees that apply to members</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <Label>Registration Fee (₦)</Label>
                    <Input type="number" step="0.01" {...register('registration_fee')} />
                  </div>

                  <div className="space-y-2">
                    <Label>Account Maintenance Fee (₦)</Label>
                    <Input type="number" step="0.01" {...register('maintenance_fee')} />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(v) => setValue('maintenance_frequency', v as 'weekly' | 'monthly')}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  <div className="flex items-center space-x-3">
                    <Switch
                      ...
                      onCheckedChange={(checked) => setValue('auto_deduct_fees', checked)}
                    />
                    <Label className="font-normal">Auto-deduct fees from member savings</Label>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <Label className="text-sm text-muted-foreground">SMS Charges</Label>
                      <RadioGroup
                        value={watch('sms_charge_bearer')}
                        onValueChange={(v) => setValue('sms_charge_bearer', v as 'cooperative' | 'member')}
                      >
                        <div className="flex items-center space-x-2 mt-2">
                          <RadioGroupItem value="cooperative" />
                          <Label className="font-normal">Cooperative pays</Label>
                        </div>
                        <div className="flex items-center space-x-2 mt-2">
                    ...
                    <div>
                      <Label className="text-sm text-muted-foreground">Email Charges</Label>
                      <RadioGroup
                        value={watch('email_charge_bearer')}
                        onValueChange={(v) => setValue('email_charge_bearer', v as 'cooperative' | 'member')}
                      >
                        <div className="flex items-center space-x-2 mt-2">
                          <RadioGroupItem value="cooperative" />
                          <Label className="font-normal">Cooperative pays</Label>
  ...
  <RadioGroup
    ...
    onValueChange={(v) => setValue('deposit_charge_bearer', v as 'cooperative' | 'member')}
  >
    <div className="flex items-center space-x-2 mt-2">
      <RadioGroupItem value="cooperative" />
      <Label className="font-normal">Cooperative absorbs fee</Label>
    </div>
    <div className="flex items-center space-x-2 mt-2">
      <RadioGroupItem value="member" />
      <Label className="font-normal">Debit member (deducted from deposit)</Label>
    </div>
  </RadioGroup>
</div>
              </CardContent>
          ...
          {activeTab === 'notifications' && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>Choose which events trigger SMS and Email alerts</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-6">
                  {[
                    ...
                    { key: 'loanRepayment', label: 'Loan Repayment Due/Received' },
                    { key: 'feeDeduction', label: 'Fee Deduction' },
                  ].map(({ key, label }) => {
                    const smsPath = `notifications.${key}.sms` as const;
                    const emailPath = `notifications.${key}.email` as const;
                    return (
  ...
  <Card>
    ...
    <CardContent className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Badge variant={currentUser?.mfa_enabled ? 'default' : 'secondary'}>
      ...
      {!currentUser?.mfa_enabled ? (
        <div className="space-y-4">
          <Alert>
            <AlertDescription>
              MFA adds strong protection to withdrawals. Once enabled, you'll need to enter a code from your authenticator app every time you withdraw funds.
            </AlertDescription>
          </Alert>
        ...
        <div className="space-y-4">
          <Alert className="border-green-600 bg-green-50">
            ...
            <AlertDescription className="text-green-800">
              MFA is active. You will be prompted for a verification code during withdrawals.
            </AlertDescription>
          </Alert>
